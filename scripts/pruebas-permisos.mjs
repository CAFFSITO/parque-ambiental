...
assert.equal(pages.length, 11, 'Cambió el inventario de páginas');
...
async function execute() {
  ...
  await runAction('simularLectura', [{ dispositivoId: sim.id, temperatura: 20, humedad: 50, boton: 'NINGUNO' }]);
  ...
  checks.push('POST ingest sin sesión: 200 con credencial; ignora área declarada y guarda área asignada.', 'Simulador aislado en /diagnostico: recibe el id de un dispositivo SIMULADO, se autentica con una credencial efímera propia y no usa DEVICE_KEY. R1 cerrado, ver 80-simulador.md.');
